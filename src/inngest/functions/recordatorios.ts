...
export const recordatorioDiaAnterior = inngest.createFunction(
  { id: "recordatorio-dia-anterior", triggers: [{ cron: "TZ=America/Argentina/Buenos_Aires 0 10 * * *" }] },
  async ({ step }) => {
...
export const recordatorio3HsAntes = inngest.createFunction(
  { id: "recordatorio-3hs-antes", triggers: [{ cron: "TZ=America/Argentina/Buenos_Aires 0 * * * *" }] },
  async ({ step }) => {
