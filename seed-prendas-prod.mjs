// Carga idempotente de prendas + duraciones estimadas en la base de PRODUCCIÓN.
// Procesos: [Manchas, Lavado, Secado, Planchado]. 0 = el proceso no aplica a esa prenda.
// Re-ejecutable: actualiza minutos y no duplica prendas (match por nombre, case-insensitive).
import { PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();

const PROCESOS = ["Manchas", "Lavado", "Secado", "Planchado"];

// mins = [Manchas, Lavado, Secado, Planchado] (estimaciones provisorias)
const PRENDAS = [
  // --- Indumentaria ---
...
async function main() {
  // 1) Asegurar los 4 procesos (find-or-create por nombre, preservando orden).
  const procesoByNombre = {};
  ...
  // Orden base: continuar después del máximo existente.
  const maxAgg = await prisma.lavPrenda.aggregate({ _max: { orden: true } });
  let nextOrden = (maxAgg._max.orden ?? -1) + 1;

  let creadas = 0, existentes = 0, duracionesUpsert = 0;

  for (const p of PRENDAS) {
    // Match case-insensitive por nombre exacto para no duplicar.
    ...
    for (let j = 0; j < PROCESOS.length; j++) {
      ...
      if (minutos > 0) {
        ...
        // El proceso no aplica: eliminar duración previa si existiera.
